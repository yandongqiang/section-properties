...
use section_properties::{
    ...
    section_library::steel::{ChannelSection, ISection, TeeSection},
    section_library::{
        CircularHollowSection, CircularSection, ParametricSection, RectangularSection,
    },
};
...
fn benchmark_circular_section() {
    ...
    let eps = CIRCLE_EPS;
    assert!(
        (props.area - expected_area).abs() <= eps,
        "area: diff={} (actual={}, expected={})",
        (props.area - expected_area).abs(),
        props.area,
        expected_area
    );
    assert!(
        (props.ix - expected_ix).abs() <= eps,
        "ix: diff={} (actual={}, expected={})",
        (props.ix - expected_ix).abs(),
        props.ix,
        expected_ix
    );
    assert!(
        (props.iy - expected_iy).abs() <= eps,
        "iy: diff={} (actual={}, expected={})",
        (props.iy - expected_iy).abs(),
        props.iy,
        expected_iy
    );
    assert!((props.ixy).abs() <= eps, "ixy: actual={}", props.ixy);
    assert!(
        (props.ix + props.iy - expected_j).abs() <= eps,
        "J: diff={} (actual={}, expected={})",
        (props.ix + props.iy - expected_j).abs(),
        props.ix + props.iy,
        expected_j
    );
    ...
    assert!(
        (props.centroid.x).abs() <= eps,
        "centroid.x: actual={}",
        props.centroid.x
    );
    assert!(
        (props.centroid.y).abs() <= eps,
        "centroid.y: actual={}",
        props.centroid.y
    );
}
...
fn benchmark_circle_approximation() {
    ...
    assert!(
        (props.iy - expected_ix).abs() <= eps,
        "iy: diff={} (actual={}, expected={})",
        (props.iy - expected_ix).abs(),
        props.iy,
        expected_ix
    );
}
...
fn benchmark_circle_convergence() {
    ...
    for n in [8, 16, 32, 64, 128] {
        ...
        let area_err =
            (props.area - std::f64::consts::PI * r * r).abs() / (std::f64::consts::PI * r * r);
...
fn benchmark_channel_section() {
    ...
    assert!(
        (props.area - 0.0022).abs() < 1e-5,
        "Channel area: actual={}",
        props.area
    );
    assert!(
        (props.ix - ixx_expected).abs() <= 1e-6,
        "Channel ix: diff={} (actual={}, expected={})",
        (props.ix - ixx_expected).abs(),
        props.ix,
        ixx_expected
    );
    assert!(
        (props.iy - iyy_expected).abs() <= 1e-6,
        "Channel iy: diff={} (actual={}, expected={})",
        (props.iy - iyy_expected).abs(),
        props.iy,
        iyy_expected
    );
...
fn benchmark_tee_section() {
    ...
    assert!(
        (props.ix - ixx_expected).abs() <= 1e-4,
        "Tee ix: diff={} (actual={}, expected={})",
        (props.ix - ixx_expected).abs(),
        props.ix,
        ixx_expected
    );
    assert!(
        (props.iy - iyy_expected).abs() <= 1e-4,
        "Tee iy: diff={} (actual={}, expected={})",
        (props.iy - iyy_expected).abs(),
        props.iy,
        iyy_expected
    );

...
fn main() {
    ...
    println!(
        ...
        n_dof + 1,
        n_dof + 1,
        A_data["data"].as_array().unwrap().len()
    );
    ...
    println!(
        "Max residual: {:.2e}, rel: {:.2e}",
        max_res,
        max_res / b_norm.max(1e-300)
    );

    // Check constraint residual C^T * omega
    let c_data: Vec<f64> =
        serde_json::from_value(load_json("rust_augmented_Angle_100x100.json")["C"].clone())
            .unwrap();
    let omega = &x[..x.len() - 1];
    let ct_omega: f64 = c_data
        .iter()
        .zip(x.iter().take(n_dof))
        .map(|(&c, &w)| c * w)
        .sum();

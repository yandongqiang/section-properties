...
use section_properties::section_library::steel::{ChannelSection, AngleSection};
...
fn assert_rel(actual: f64, expected: f64, tol: f64, name: &str) {
    ...
    println!("  {}: actual={:.6e} expected={:.6e} err={:.3e}", name, actual, expected, err);
...
fn run_scale_test(name: &str, section: Section, scales: &[f64]) {
    ...
    for &s in scales {
        ...
        let result = match compute_fem_warping_solution(&scaled_section, &scaled_props, base_nu, MeshControl::Fine) {
            Ok(r) => r,
            Err(e) => {
        ...
        println!("Shear center: ({:.6}, {:.6})", result.shear_center.x, result.shear_center.y);
        println!("omega_max: {:.6e}", result.omega_max);
        println!("Exact: {}, Reg: {}", result.used_exact_solver, result.used_regularization);
        println!("Exact residual: {:.2e}, Reg residual: {:.2e}", 
    ...
    let base_idx = results.iter().position(|(s, _)| (s - 1.0).abs() < f64::EPSILON);
    if base_idx.is_none() {
    ...
    for (s, result) in &results {
        ...
        println!("  J: {:.6e} vs {:.6e} (err={:.2e})", result.j, base_j * s.powi(4), j_err);
        println!("  Iw: {:.6e} vs {:.6e} (err={:.2e})", result.iw, base_iw * s.powi(6), iw_err);
        println!("  SC: ({:.6e}, {:.6e}) vs ({:.6e}, {:.6e}) (err x={:.2e}, y={:.2e})", 
            result.shear_center.x, result.shear_center.y, 
            base_sc_x * s, base_sc_y * s, sc_x_err, sc_y_err);
        println!("  omega_max: {:.6e} vs {:.6e} (err={:.2e})", 
            result.omega_max, base_omega_max * s * s, omega_max_err);
        ...
        assert!(sc_x_err_val < 1.0 || (base_sc_x * s).abs() < 1e-6, 
            "Shear centre X error too large: {:.2e}", sc_x_err_val);
        assert!(sc_y_err_val < 1.0 || (base_sc_y * s).abs() < 1e-6,
            "Shear centre Y error too large: {:.2e}", sc_y_err_val);
        assert!(omega_max_err_val < 1.0, "omega_max error too large: {:.2e}", omega_max_err_val);

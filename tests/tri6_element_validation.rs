...
fn tri6_unit_triangle_element_matrices() {
    ...
    let expected_c_el = [
        0.0,
        0.0,
        0.0, // nodes 0,1,2 (corners) - limitation of 4-point rule
        1.0 / 6.0,
        1.0 / 6.0,
        1.0 / 6.0, // nodes 3,4,5 (mid-sides) - 4-point rule gives 2× theoretical
    ];
    ...
    for i in 0..6 {
        println!(
            "  C[{}] = {:.10}  (expected {:.10})  diff={:.2e}",
            i,
            c_el[i],
            expected_c_el[i],
            (c_el[i] - expected_c_el[i]).abs()
        );
    }
...
fn tri6_equilateral_triangle_element_matrices() {
    ...
    let c_sum: f64 = c_el.iter().sum();
    println!(
        "Sum C_e = {:.10} (expected full area = {:.6})",
        c_el.iter().sum::<f64>(),
        3.0_f64.sqrt()
    );
    ...
    assert!(
        (c_el.iter().sum::<f64>() - expected_sum).abs() < 1e-10,
        "C sum = {} (expected {})",
        c_el.iter().sum::<f64>(),
        expected_sum
    );

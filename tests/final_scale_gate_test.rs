...
fn test_dense_solver_extreme_scales() {
    ...
    let scales = [1e-12, 1e-9, 1e-6, 1e-3, 1e-2, 1e-1, 1.0, 1e1, 1e2, 1e3, 1e6, 1e9, 1e12];
    ...
    for &alpha in &scales {
        ...
        let solver = SkylineLdlt::factor(&k).expect(&format!("Factorization failed at α={:.0e}", alpha));
        
        // Check scale factor stored
        println!("\nα = {:.0e}: stored scale = {:.2e}", alpha, solver.scale());
        
        let x = solver.solve(&f_data).expect(&format!("Solve failed at α={:.0e}", alpha));
        ...
        assert!(max_err < 1e-10, "Scale invariance violated at α={:.0e}: max_err={:.2e}", alpha, max_err);
    }
}
...
fn test_skyline_ldlt_extreme_scales() {
    ...
    for &alpha in &scales {
        ...
        let pivot_tol = 1e-15 * solver.scale().max(1.0);
        println!("α = {:.0e}: scale={:.2e}, pivot_tol={:.2e}, max_err={:.2e}", 
                 alpha, solver.scale(), pivot_tol, max_err);
        
        assert!(max_err < 1e-10, "SkylineLdlt scale invariance violated at α={:.0e}: max_err={:.2e}", alpha, max_err);
    }
}
...
fn test_lagrange_extreme_scales() {
    ...
    for &alpha in &scales {
        ...
        let u = solver.solve_lagrange(&c_data, &f_data).expect(&format!("Lagrange solve failed at α={:.0e}", alpha));
        ...
        println!("α = {:.0e}: scale={:.2e}, near_zero_tol={:.2e}, max_err={:.2e}", 
                 alpha, solver.scale(), near_zero_tol, max_err);
        
        assert!(max_err < 1e-10, "Lagrange scale invariance violated at α={:.0e}: max_err={:.2e}", alpha, max_err);
    }
}
...
fn test_sparse_lu_extreme_scales() {
    ...
    for &alpha in &scales {
        ...
        let lu = SparseLu::factor(&a).expect(&format!("SparseLU factorization failed at α={:.0e}", alpha));
        
        let x = lu.solve(&b_data).expect(&format!("SparseLU solve failed at α={:.0e}", alpha));
        ...
        assert!(max_err < 1e-10, "SparseLU scale invariance violated at α={:.0e}: max_err={:.2e}", alpha, max_err);
    }
}
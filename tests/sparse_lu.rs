...
fn test_multiple_pivots() {
    ...
    // Verify PA = LU
    let max_diff = lu.verify_pa_eq_lu(&build_dense(3, &[
        1e-14, 1.0, 2.0,
        1.0, 1.0, 3.0,
        0.0, 1.0, 1.0,
    ]));
        ...
        for seed in 0..5 {
            ...
            match SparseLu::factor(&a) {
                Ok(lu) => {
                    ...
                    assert!(max_diff < 1e-10, "PA != LU for n={}, seed={}: max_diff = {}", n, seed, max_diff);
...
fn test_scale_invariance() {
    // Test that the solver works correctly for a well-conditioned matrix
    // The actual scale invariance test is done with Python's scipy solver in solve_validation.py
    // This test just verifies the solver works correctly for a well-conditioned matrix
    let a = build_dense(3, &[
        4.0, 1.0, 0.0,
        1.0, 4.0, 1.0,
        0.0, 1.0, 4.0,
